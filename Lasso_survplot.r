...
train_data <- read.csv("Lasso_TCGA_dataALL.csv")
...
pdf(paste("Lasso_","TCGA","_survplot.pdf"))  
...
for (k in 1:dim(dataset)[1]){
  ...
  test_data<- read.csv(paste("Lasso_",gse_id,"_dataALL.csv",sep=""),header=T,row.names=1,
                       stringsAsFactors=FALSE)
  
  pdf(paste("Lasso_",gse_id,"_survplot.pdf"))  
